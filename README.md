...
Training data contains images captured from the simulator, paired with the corresponding steering angl. Training with data from only the human driver is not sufficient; the network must also learn how to recover from any mistakes, or the car will slowly drift off the road. The training data is therefore augmented with additional images that show the car in different shifts from the center of the lane and rotations from the direction of the road.
...
* Driving around lane1 to capture images with (30mph) till the model is enough good to drive at (15mph), It took me about 5 laps with using he mouse control. 
* Driving around lane2 to capture images with (15mph) till the model is enough good to drive at (10mph), It took me about 2 laps with using he mouse control. 
...
Please note that the batch data is generated by generator and also all mendioned data augmentation operations are applyed for the training set, but only flip and crop for validation.
...
##### Data Augmentation
* The augmented images helped the model to generalize better so the car drives smooth on both lanes, and it also drives great on third lane where I didn't provide any training data, but the model is robust to drive in an unknown lane.
